...
async function sendTGWithAi(ai: Ai, reminder: string, time12: string): Promise<string> {
	...
	const promptLines = ["請扮演一位亞洲父母，角色設定如下：", "- 嚴厲、講求紀律，對時間與規矩非常敏感", "- 成就至上，視孩子的未來為己任", "- 語氣關切但強硬，經常以「你知道的，這是為你好！」作結", "- 偏好用命令式語氣給出提醒與指示", "- 偶爾會加上一兩句過去的辛酸歷程，強化正當性", "規則：1.僅用繁體中文；2.使用全型標點符號；3.每句可附加 0–3 個表情符號，但不能破壞嚴肅口吻；", "4.回應內容需同時包含『時間』與『提醒文字』；5.時間請使用中文 12 小時制（例如：下午三點、早上七點）；", `時間：${time12}`, `提醒的文字：${reminder}`];
	const messages = [
		{
			role: "system",
